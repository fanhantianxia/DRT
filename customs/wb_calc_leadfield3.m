function [leadfield,headmodel] = wb_calc_leadfield3(xyz_elec,xyz_dipoles,xyz_dipOri,headmodel)
% Forward program:  calculate electric leadfield matrix for dipoles in concentric spheres
%  based on normalized associated Legendre fucntion.
% -------------------------------------------------------------------------
% Input:
...
if isempty(xyz_dipOri)
    ...
    leadfield_temp.X = reshape(leadfield ( :, 1, : ),Nelec,Ndips);
    leadfield_temp.Y = reshape(leadfield ( :, 2, : ),Nelec,Ndips);
    leadfield_temp.Z = reshape(leadfield ( :, 3, : ),Nelec,Ndips);
    leadfield = leadfield_temp;
else
