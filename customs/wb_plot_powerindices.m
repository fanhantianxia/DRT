function wb_plot_powerindices(EEG_results,outputdir,filename)
...
if ~isempty(EEG_results) && isfield(EEG_results,'type')
    if isequal(EEG_results.type,'power')
                ...
                try
                    ...
                    for j1 = 1:Nf
                        if any(~isnan(EEG_results.Power_mean(:,j1)))
                            ...
                            topoplot(EEG_results.Power_mean(:,j1),chanlocs,'shrink','off','maplimits','maxmin');
                            axis equal;
