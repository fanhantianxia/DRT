function EEG_results = wb_calculate_EEGnetwork_ADTF(EEG,epochLenth,eventlabel,bandLimit,selechanns,method,proportion)
...
%    epochLenth: length of small epochs to calculate EEG network (no overlapped).
%         unit is second.Default is 5s.
...
%  Output:
%    EEG_network: results including connection matrices, mean connection 
...
% proportion
if proportion < 0 || proportion >= 1 || ~isfinite(proportion);
    proportion = 0;
    disp('Input proportion is not correct, set proportion = 0');
end
    ...
    if isfield(EEG,'event')
        ...
        if ~isempty(allevents)
            ...
            if ischar(allevents(1).type)
                ...
                LogiVal = cellfun(@(x) isequal(x,eventlabel), Eventtypes);
                LogiVal_badblock = cellfun(@(x) isequal(x,'9999'), Eventtypes); % bad block
            else
                ...
                if isnan(str2double(eventlabel))
                    LogiVal = Eventtypes == eventlabel;
                else
                    LogiVal = Eventtypes == str2double(eventlabel);
                end
                
                try
                    LogiVal_badblock = Eventtypes == 9999; % bad block
                catch
                    LogiVal_badblock = Eventtypes == str2double('9999');
                end
                
            end;
            IndexInd = TypeInd(1,LogiVal); % find eventlabel
            IndexInd_badblock = TypeInd(1,LogiVal_badblock); % find bad block
            if ~isempty(IndexInd)
